...
export const api = {
  ...
  saveNote: async (slug, content) => {
    const data = await request(`/notes/${slug}`, { method: 'PUT', body: JSON.stringify({ content }) })
  ...
  setPassword: (slug, password) => request(`/notes/${slug}/password`, { method: 'POST', body: JSON.stringify({ password }) }),
  verifyPassword: (slug, password) => request(`/notes/${slug}/verify-password`, { method: 'POST', body: JSON.stringify({ password }) }),
  renameNote: (slug, newSlug) => request(`/notes/${slug}/rename`, { method: 'POST', body: JSON.stringify({ newSlug }) }),
  setAccessMode: (slug, mode) => request(`/notes/${slug}/access-mode`, { method: 'POST', body: JSON.stringify({ mode }) }),
